...
class JbdBt(Battery):
    def __init__(self, address, config_path=None):
        ...
        # CRITICAL FIX: Initialize required battery voltage attributes EARLY
        # These are required by the D-Bus service creation and must have actual values
        self.cell_count = 16  # Default, will be updated from BMS data
        self.min_battery_voltage = MIN_CELL_VOLTAGE * self.cell_count  # Required for D-Bus
        self.max_battery_voltage = MAX_CELL_VOLTAGE * self.cell_count  # Required for D-Bus
        ...
        self.dev.addGeneralDataCallback(self.generalDataCB)  # Fixed method name
    ...
    def read_gen_data(self):
        ...
        try:
            ...
            # CRITICAL: Update min/max voltage based on actual cell count from BMS
            self.max_battery_voltage = MAX_CELL_VOLTAGE * self.cell_count
            self.min_battery_voltage = MIN_CELL_VOLTAGE * self.cell_count

            logger.debug(f"Updated voltage limits for {self.address}: Min={self.min_battery_voltage:.2f}V, Max={self.max_battery_voltage:.2f}V ({self.cell_count} cells)")
